<?php
/**
* Plugin Name: miniOrange OAuth 2.0 Server
	...
	function miniorange_oauth_save_settings(){
		...
		if( isset( $_POST['option'] ) and $_POST['option'] == "mo_oauth_verify_customer" ) {	//register the admin to miniOrange
			...
			if($this->mo_oauth_check_empty_or_null($_POST['mo_oauth_custom_client_name']) || $this->mo_oauth_check_empty_or_null($_POST['mo_oauth_client_redirect_url'])) {
				...
				$db->exec('INSERT INTO oauth_clients (client_name, client_id, client_secret, redirect_uri) VALUES ("'.$client_name.'", "'.generateRandomString(30).'", "'.generateRandomString(30).'", "'.$redirect_url.'")');
			...
			if($this->mo_oauth_check_empty_or_null($_POST['mo_oauth_custom_client_name']) || $this->mo_oauth_check_empty_or_null($_POST['mo_oauth_client_redirect_url'])) {
				...
				$dbfile = __DIR__.'/data/oauth.sqlite';
				$db = new PDO(sprintf('sqlite:%s', $dbfile));
				$db->exec('UPDATE oauth_clients SET redirect_uri = "'.$redirect_url.'" WHERE client_name = "'.$client_name.'"');
				update_option( 'message', 'Your settings are saved successfully.' );
				$this->mo_oauth_show_success_message();
			}
		}
			...
			update_option( 'message', 'Your settings are saved successfully.' );
			$this->mo_oauth_show_success_message();
			wp_redirect('admin.php?page=mo_oauth_server_settings&action=update&app='.urlencode($appname));
		}
		//submit google form
