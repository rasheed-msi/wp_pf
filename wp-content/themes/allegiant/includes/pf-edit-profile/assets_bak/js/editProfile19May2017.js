...
app.controller('TabsCtrl', ['$scope', '$http', '$location', function ($scope, $http, $window, $location) {
	 ...
	 $scope.show = false;
        ...
        $scope.onTabSelected = function (index) {
            $scope.show = false; //show unsaved data on tab change
            if ($scope.formStatus === true) {
                $scope.show = true;
//		dhtmlx.confirm("Do you really want to move to next tab>", function (result) {
//                    if(result == true){
//console.log('hhh1');
//			$scope.model.tabs[index].isLoaded = true;
//		    }else{
//			console.log('hhh2');
//			$scope.model.tabs[2].isLoaded = true;
//			$scope.model.tabs[2].templateUrl = 'assets/views/contact_info.html';
//		    }
//                });
            }else{
	         console.log('hhh3');
...
app.controller('childPrefCntrl', ['$scope', '$http', function ($scope, $http, $window) {
        ...
        $scope.submit = function () { console.log('hhhh');
            //$scope.$parent.newFn(false);
            var req = {
                method: 'POST',
